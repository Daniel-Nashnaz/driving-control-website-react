...
const LastTravelOfUsers = () => {
    ...
    return (<>
            <List
              header={<div style={{ textAlign: 'center' }}><i><h4 style={{ color: 'gold' }}>Last Travels </h4></i></div>}
                ...
                >
                  <Skeleton avatar title={"Last Travels"} loading={item.loading} active>
                    <List.Item.Meta
                      ...
                      title={item.fullName}
                      description={`Time Start: ${formatTimeWithAM_PM(item.timeStart)}, Username: ${item.userName}, Email: ${item.email}, Phone: ${item.phone}`}
                    />
                  </Skeleton>
                </List.Item>
              )}
